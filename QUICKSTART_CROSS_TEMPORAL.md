...
複数プロジェクト対応の IRL で、0-3m の期間で訓練し、0-3m, 3-6m, 6-9m, 9-12m を予測する全 10 パターンのクロス評価とヒートマップを作成します。
...
デフォルト設定（既存の results/review_continuation_cross_eval_nova/と同じ期間）:

- レビュー CSV: `data/review_requests_openstack_multi_5y_detail.csv`
- 訓練期間ベース: `2021-01-01` → 2021-01-01 ～ 2022-01-01（12 ヶ月）
- 評価期間ベース: `2023-01-01` → 2023-01-01 ～ 2024-01-01（12 ヶ月）
- 総期間: 12 ヶ月（4 期間 × 3 ヶ月）
...
1. CPU で実行（自動フォールバック）
2. hidden_dim を減らす（コード内で 128→64 に変更）
...
- [既存結果の例](results/review_continuation_cross_eval_nova/README.md)
...
A: 訓練期間 ≤ 評価期間の制約により、4+3+2+1 = 10 パターンになります。全 16 パターン（4×4）ではありません。
