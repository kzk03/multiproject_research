"""
Random Forest Nova単体評価 - 案1: IRL完全再現 (スライディングウィンドウ)
...
def extract_samples_at_cutoff(
    ...
    Args:
        cutoff_date: カットオフ日
        history_months: 履歴期間（月数）
        future_months: ラベル期間（月数）
        extended_months: 拡張期間（月数）
    """
...
def generate_sliding_window_samples(
    ...
    logger.info(f"  開始: {start_dt.strftime('%Y-%m-%d')}")
    logger.info(f"  終了: {end_dt.strftime('%Y-%m-%d')}")
    logger.info(f"  スライド間隔: {slide_months}ヶ月")

    for i, cutoff_dt in enumerate(cutoff_dates, 1):
        ...
        if i % 5 == 0 or i == len(cutoff_dates):
            logger.info(f"  時点 {i}/{len(cutoff_dates)}: {cutoff_dt.strftime('%Y-%m-%d')} → {len(samples)}サンプル")
...
def evaluate_pattern_case1(df, train_window_months, eval_start, eval_end, min_history_requests=1):
    """
    案1: スライディングウィンドウで訓練、IRLと同じ評価

    Args:
        train_window_months: 訓練ラベル窓の長さ (3ヶ月)
        eval_start: 評価開始日
        eval_end: 評価終了日
    """
    # 訓練データ生成 (2019-01-01 ~ 2021-01-01をスライド)
    train_samples = generate_sliding_window_samples(
        df,
        start_date='2019-01-01',
        end_date='2021-01-01',
        window_months=train_window_months,
...
def main():
    logger.info("=" * 80)
    logger.info("RF 案1: IRL完全再現 (スライディングウィンドウ) - Nova単体")
    logger.info("訓練: スライディングウィンドウで生成 (~1000サンプル)")
    ...
    # 10パターン評価
    eval_patterns = [
        ("2023-01-01", "2023-04-01", 3, "0-3m"),
        ("2023-04-01", "2023-07-01", 3, "3-6m"),
        ("2023-07-01", "2023-10-01", 3, "6-9m"),
        ("2023-10-01", "2024-01-01", 3, "9-12m"),
    ]

    results = []

    for i, (eval_start, eval_end, window_months, name) in enumerate(eval_patterns, 1):
        logger.info("\n" + "=" * 80)
        logger.info(f"評価パターン {i}/4: {name}")
        logger.info("=" * 80)
    ...
    # 結果保存
    output_dir = Path("/Users/kazuki-h/research/multiproject_research/outputs/rf_nova_case1_sliding")
    ...
    # サマリー
    logger.info("\n" + "=" * 80)
    logger.info("案1: スライディングウィンドウ - サマリー")
    logger.info("=" * 80)
