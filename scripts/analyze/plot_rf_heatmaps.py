# -*- coding: utf-8 -*-
"""
ヒートマップ作成用スクリプト（RF Nova 行列を結合して描画）
- 入力: performance/rf/ 配下の matrix_*.csv（AUC_ROC, F1, PRECISION, RECALL）
- 出力: heatmap_{AUC_ROC,F1,PRECISION,RECALL}.png と heatmap_4_metrics.png
- 利用例: uv run scripts/analyze/plot_rf_heatmaps.py --input-dir outputs/rf_vs_irl_nova_summary/performance/rf --output-dir outputs/rf_vs_irl_nova_summary/performance/rf
"""
...
PanelTitle = "RF: クロス評価ヒートマップ"

FileNameMap = {
    "AUC_ROC": "rf_matrix_AUC_ROC.csv",
    "F1": "rf_matrix_F1.csv",
...
def main() -> None:
    parser = argparse.ArgumentParser(description="RF Nova 行列からヒートマップを生成する")
    parser.add_argument(
        "--input-dir",
        type=pathlib.Path,
        default=pathlib.Path("outputs/rf_vs_irl_nova_summary/performance/rf"),
    ...
    # 4面まとめは RFheatmap.{png,pdf}
    plot_four_panel(matrices, args.output_dir / "RFheatmap", vmin, vmax)
