# Review Continuation Cross-Evaluation Results (Nova)

## ディレクトリ構造

```
review_continuation_cross_eval_nova/
├── README.md                                      # このファイル
...
2. **[ヒートマップ深層考察\_文章版.md](ヒートマップ深層考察_文章版.md)** ⭐⭐⭐ 超詳細分析（物語形式）
...
4. **[開発者特性別予測精度\_詳細分析レポート.md](開発者特性別予測精度_詳細分析レポート.md)** ⭐⭐⭐ 定量的分析
...
5. **[時間的非対称性の考察.md](時間的非対称性の考察.md)** ⭐⭐⭐ 因果性分析
   ...
   - 因果的決定論と時間の矢の視点からの考察
   - 早期介入の重要性（初期 3 ヶ月で全てが決まる）
   - 物理学・心理学との類似性
   - 30-45 分で因果メカニズムを理解できる

6. **[RQ 分析レポート.md](RQ分析レポート.md)** ⭐ 詳細分析
...
7. **可視化図表**
   - [heatmaps/heatmap_4_metrics.png](heatmaps/heatmap_4_metrics.png) - 4 メトリクス統合
...
- **訓練**: 2021-01-01 ～ 2023-01-01（24 ヶ月）
- **評価**: 2023-01-01 ～ 2024-01-01（12 ヶ月）

### モデル

- **アーキテクチャ**: IRL + LSTM
- **状態特徴量**: 10 次元（経験、活動、協力、品質など）
- **行動特徴量**: 4 次元（強度、協力、応答速度、規模）
- **隠れ層**: 128 ユニット、Dropout 0.2
...
- 平均 AUC-ROC 0.754（優れた予測）
- 最高 AUC-ROC 0.910（train: 0-3m, eval: 6-9m）
- 汎化性能優秀（訓練-評価差 0.009）
...
- 訓練 3-6m: AUC-ROC 0.820（最高性能）
- 評価 6-9m: 最も予測しやすい（平均 0.824）

⚠️ **長期期間（9-12 ヶ月）は性能低下**
...
uv run python scripts/training/irl/train_irl_review_acceptance.py \
  ...
  --eval-end 2024-01-01 \
   --output outputs/review_continuation_cross_eval_nova
```
...
model = RetentionIRLSystem.load_model(
   'outputs/review_continuation_cross_eval_nova/train_3-6m/irl_model.pt'
)
...
- [ヒートマップ深層考察\_文章版.md](ヒートマップ深層考察_文章版.md) - ヒートマップと特徴量の徹底分析・物語形式（最推奨）⭐⭐⭐
- [ヒートマップ深層考察レポート.md](ヒートマップ深層考察レポート.md) - ヒートマップと特徴量の徹底分析・構造的（超推奨）⭐⭐
- [開発者特性別予測精度\_詳細分析レポート.md](開発者特性別予測精度_詳細分析レポート.md) - 定量的分析・実装ガイド（超推奨）⭐⭐⭐
- [時間的非対称性の考察.md](時間的非対称性の考察.md) - 因果性分析（超推奨）⭐⭐⭐
- [RQ 分析レポート.md](RQ分析レポート.md) - 詳細分析（推奨）
- [総合分析レポート.md](総合分析レポート.md) - 全体分析
- [developer_analysis_charts/](developer_analysis_charts/) - 開発者特性別の詳細グラフ・表（NEW!）⭐⭐⭐
- [../../../README_TEMPORAL_IRL.md](../../../README_TEMPORAL_IRL.md) - Temporal IRL 全般
